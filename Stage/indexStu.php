...
//condition de clicker sur ACTIVER 
  if (isset($_POST["activer"])){
   //la session commence pour la sauvegarde des données
    ...
    $req = "SELECT * FROM Etudiant WHERE NoDA='". $_POST["NoDA"] ."' AND  Code='".$_POST["Code"]."'";
    ...
    if ($reqVEF2 > 0) {
        while ( $tab = mysqli_fetch_array($reqVEF) ){
            ...
            $_SESSION["session"] = $tab[3];
            $_SESSION["code"] =$tab[4];
            $_SESSION["formation"] =$tab[5];
            $_SESSION["groupe"]   = $tab[6];
            $_SESSION["photo"]    = $tab[7];
...
<html>
    ...
    <body>
        <div id="dvP">
          ...
          <br><br><br><br>
          <p>No de DA</p>
          <form method="POST" action="indexStu.php">
